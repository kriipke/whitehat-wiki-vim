...
" l0xy:
"   custom
syn region tag_kw matchgroup=FoldColumn start="<kw>" end="</kw>" contains=ALLBUT,tag_kw,@dokuwikiNoneTextItem extend concealends
syn region tag_txt matchgroup=FoldColumn start="<txt>" end="</txt>" contains=ALLBUT,tag_txt,@dokuwikiNoneTextItem extend concealends
syn region tag_cmd matchgroup=FoldColumn start="<cmd>" end="</cmd>" contains=ALLBUT,tag_cmd,@dokuwikiNoneTextItem extend concealends
syn region tag_path matchgroup=FoldColumn start="<path>" end="</path>" contains=ALLBUT,tag_path,@dokuwikiNoneTextItem extend concealends

" l0xy:
"   plugins
syn region plugin_sxh matchgroup=FoldColumn start="<sxh>" end="</sxh>" contains=ALLBUT,plugin_sxh,@dokuwikiNoneTextItem extend concealends
...
syn region dokuwikiInternalLink matchgroup=dokuwikiLink start="\[\[" end="\]\]" contains=@NoSpell,dokuwikiLinkMedia,dokuwikiLinkNoMedia,dokuwikiLinkRef keepend extend


syn region dokuwikiLinkMedia matchgroup=dokuwikiLink start="|{{"ms=s-1,rs=s+1 end="}}\]\]"me=e-2,re=e-2 contained contains=dokuwikiInternalMediaLink,dokuwikiLinkCaption,dokuwikiLinkEndBrackets keepend
...
"l0xy: added nextgroup=dokuwikiLinkEndBrackets to allow for line below this
syn region dokuwikiLinkCaption start="." end="\]\]"me=e-2 contained nextgroup=dokuwikiLinkEndBrackets
"l0xy: remotes one of the two end brackets in internal link (one of the two opening brackets is removed in the `hs=s+1` statement instead of `hs=s+2` in the dokuwikiInternalLink match lines)
syn region dokuwikiLinkEndBrackets start="\]\]\@=" end="\]"ms=e-2,me=e-1 contained conceal
...
" l0xy:
" link WITHOUT caption
syn match dokuwikiInternalLink "\[\[[^|\]]*[:#]\([|a-zA-Z0-9_]*\)\@="hs=s+1,he=e contained nextgroup=dokuwikiLinkCaption conceal
" link WITH caption
syn match dokuwikiInternalLink "\[\[[^|]*\s\=|"hs=s+1,he=e contained nextgroup=dokuwikiLinkCaption conceal
...
syn region dokuwikiCodeBlock    matchgroup=dokuwikiCodeMark start="<\z(code\|sxh\|file\)\(\( [[:alpha:]-]\+\( [^>]\+\)\?\)\?>\)\@="   end="</\z1>"        contains=@dokuwikiCodeBody,dokuwikiCodeBlock2 keepend extend
syn region dokuwikiCodeBlock2   matchgroup=dokuwikiCodeLang start="\(<\z(code\|sxh\|file\)\)\@<= [[:alpha:]-]\+\(\( [^>]\+\)\?>\)\@=" end="\(</\z1>\)\@=" contains=@dokuwikiCodeBody,dokuwikiCodeBlock3 keepend extend
syn region dokuwikiCodeBlock3   matchgroup=dokuwikiCodeFile start="\(<\z(code\|sxh\|file\) [[:alpha:]-]\+\)\@<= [^>]\+>\@="           end="\(</\z1>\)\@=" contains=@dokuwikiCodeBody keepend extend
syn region dokuwikiCodeBlockEnd matchgroup=dokuwikiCodeMark start="\(<\z(code\|sxh\|file\)\( [[:alpha:]-]\+\( [^>]\+\)\?\)\?\)\@<=>"  end="\(</\z1>\)\@=" keepend extend
...
hi def dokuwikiBold term=bold cterm=bold ctermfg=gray
hi def dokuwikiItalic ctermfg=gray
...
hi def tag_path term=bold cterm=bold ctermfg=5 gui=bold guifg=#ff00ff

"l0xy:
"  plugins
hi def plugin_sxh ctermfg=black ctermbg=magenta gui=bold guifg=#ff00ff
